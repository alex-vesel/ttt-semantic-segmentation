...
test_dataset = CityscapesDataset(split='val', img_size=img_size, foggy=False, rotate=False, fog_level=-1)
...
total_accuracy = 0
total_loss = 0
for i, (images, labels, rot_label) in enumerate(test_loader):
    ...
    # plt.imshow(test_dataset.unnormalize(rot_pred[0].detach().cpu()))
    # plt.show()

    # plt.imshow(test_dataset.unnormalize(images_orig[0].detach().cpu()))
    # plt.show()

    # plt.imshow(predicted[0].detach().cpu())
    # plt.show()

    # plot original image, rot_pred, label, and predicted on one plot using subplot
    # print(torch.mean(images_orig[0].detach().cpu()))
    # plt.subplot(1, 4, 1)
    # plt.imshow(test_dataset.unnormalize(images_orig[0].detach().cpu()))
    # plt.axis('off')
    # plt.subplot(1, 4, 2)
    ...
    # plt.axis('off')
    # plt.subplot(1, 4, 3)
    # plt.imshow(class_labels[0].detach().cpu())
    # plt.axis('off')
    # plt.subplot(1, 4, 4)
    # plt.imshow(predicted[0].detach().cpu())
    # plt.axis('off')
